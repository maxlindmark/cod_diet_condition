...
# Start with fitting a simple gamma model
...
## Add random `year` effect to each time_period and random `ices_rect` 

```{r random year and rectangle effects, cache=TRUE}
m02 <- brm(FR_tot ~ 0 + pred_weight_g + depth2_sc + time_period + (0 + time_period|year) + (0 + time_period|ices_rect),
           data = d, family = hurdle_gamma(link = "log", link_shape = "log", link_hu = "logit"),
           chains = 2, iter = 4000, cores = 4, inits = "0",
           control = list(adapt_delta = 0.99, max_treedepth = 15))

test <- brm(FR_tot ~ 0 + pred_weight_g + depth2_sc + time_period + (0 + time_period|year) + (0 + time_period|ices_rect),
           data = d, family = hurdle_gamma(link = "log", link_shape = "log", link_hu = "logit"),
           chains = 1, iter = 1, cores = 1, inits = "0",
           control = list(adapt_delta = 0.99, max_treedepth = 15))
...
color_scheme_set("gray")
yrep_hu_gamma <- posterior_predict(m02, draws = 500)
y <- d$FR_tot
